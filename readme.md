...
- **Review**
  - индекс по `target_user_id` для получения отзывов о пользователе
  - индекс по `author_user_id` для поиска отзывов, оставленных пользователем
...
Сущности **User**, **Card**, **Review**, **UserSession**, **UserMetrics** шардируем по `user_id` с использованием хэширования. Хэширование по `user_id` позволяет равномерно распределить пользователей между шарами. Это гарантирует, что нагрузка будет распределена и предотвращает возможные горячие шарды.
