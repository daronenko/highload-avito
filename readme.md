...
![Логическая схема БД](img/db/dbdiagram-schema.svg)

#### Описание таблиц

| **Таблица** | **Описание** |
| - | - |
| **User** | Данные пользователя, в том числе url на аватарку |
| **Avatar** | Файлы с аватаркой. Использует S3 хранилище. Пользователю отдаем url, с помощью которого получаем подгружаем аватарку на клиенет |
| **Session** | Сессии пользователей для аутентификации |
| **Favorite** | Избранные объявления каждого пользователя |
| **Category** | Возможные категории объявлений, представленные в иерархической структуре. Использует MongoDB. В поле `attributes` содержит специфичные для каждой категории характеристики товара |
| **AnnouncementAttributes** | Конкретные значения характеристик товара. Использует MongoDB. Список возможных характеристик, то есть схема, содержится в **Category** |
| **Announcement** | Информация об объявлении. Поле `image_urls` содержит массив url изображений, прикрепленных к объявлению, что позволяет избавиться от join'а для получения изображений |
| **AnnouncementImage** | Файлы с изображениями, прикрепленными к объявлению. Использует S3 хранилище |
| **AnnouncementSearch** | Индекс для поиска объявлений. Обновляется асинхронно после редактирования данных объявленияю Использует Elasticsearch |
| **Feedback** | Тело отзыва, оценка и список url изображений, прикрепленными к отзыву о продавце/покупателе |
| **FeedbackImage** | Файлы с изображениями, прикрепленными к отзыву о продавце/покупателе. Использует S3 хранилище |
...
| - | :-: | :-: | :-: | :-: |
| **User** | 256 | 0.12 | 6 | 43 |
| **Avatar** | 15376 | 6.2 | 5 | 5036 |
| **Session** | 147 | 0.6 | 49 | 4164 |
| **Favorite** | 48 | 1.1 | 287 | 2870 |
| **Category** | 256 | 0 | 0 | 32 |
| **AnnouncementAttributes** | 256 | 3.6 | 173 | 2870 |
| **Announcement** | 1024 | 14.3 | 173 | 2870 |
| **AnnouncementImage** | 51216 | 387.4 | 94 | 14350 |
| **AnnouncementSearch** | 256 | 3.6  | 173 | 1592 |
| **Feedback** | 256 | 0.008 | 0.4 | 1722 |
| **FeedbackImage** | 51216 | 0.8 | 0.2 | 517 |
...
- Список ссылок на изображения `image_urls` объявления **Announcement** и отзыва **Feedback** должен быть синхронизирован с S3, то есть записываем url в БД только после успешной загрузки в S3 хранилище
- Удаление объявления должно удалять все связанные избранные записи в **Favorite**

##### Eventual consistency

- Допустимо асинхронное удаление избранных объявлений **Favorite** и отзывов пользователя **Feedback** при удалении пользователя **User**
- Допустимо асинхронное удаление отзыва **Feedback**, характеристик объявлений **AnnouncementAttributes** и поискового индекса **AnnouncementSearch** при удалении объявления **Announcement**
- Допустимо асинхронное удаление аватарки **Avatar** в S3 при удалении пользователя **User**
- Допустимо асинхронное удаление изображений объявления **AnnouncementImage** в S3 при удалении объявления **Announcement**
- Допустимо асинхронное удаление изображений отзыва **FeedbackImage** в S3 при удалении объявления **Feedback**
- Допустимо асинхронное объявление поискового индекса **AnnouncementSearch** при обновлении данных объявления **Announcement**

#### Особенности распределения нагрузки по ключам

Многие таблицы (**Session**, **Favorite**, **Feedback**, **Avatar**) используют `user_id` для связи с пользователем, что делает его ключевым параметром для масштабирования. Шардирование по `user_id` позволит равномерно распределить нагрузку между узлами.

Объявления являются основными сущностями системы, поэтому связанные с ними таблицы (**AnnouncementAttributes**, **Feedback**, **Favorite**) могут также стать узким местом. Разбиение по `announcement_id` позволит равномерно распределить запросы, связанные с объявлениями. Популярные объявления и связанные данные можно кэшировать, чтобы ускорить выдачу.
