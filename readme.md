...
- [Avito](#avito)
  ...
  - [Основная часть](#основная-часть)
    ...
    - [6. Физическая схема БД](#6-физическая-схема-бд)
      ...
      - [Балансировка запросов и мультиплексирование подключений](#балансировка-запросов-и-мультиплексирование-подключений)
...
| **RawUserMetrics** | Необработанные метрики пользовательской активности, используемые для последующего анализа |
| **UserMetrics** | Агрегированные пользовательские метрики, используемые для рекомендаций |
...
| **RawUserMetrics** | 116 | 44.3 | 4749 | 5000 |
...
- Допустимо асинхронное
  ...
  - удаление данных в **RawUserMetrics** и **UserMetrics** при удалении пользователя **User**

#### Особенности распределения нагрузки по ключам

Многие таблицы (**UserSession**, **RawUserMetrics**, **UserMetrics**, **Event**, **Review**, **ChatMessage**) используют `user_id` для связи с пользователем, что делает его ключевым параметром для масштабирования. Шардирование по `user_id` позволит равномерно распределить нагрузку между узлами.

Объявления **Card** являются основными сущностями системы, поэтому связанные с ними таблицы (**CardSearch**, **Review**, **ModerationQueue**) могут также стать узким местом. Разбиение по `card_id` позволит равномерно распределить запросы, связанные с объявлениями.
...
| - | - | - |
| **PostgreSQL** | User, Card, Review, Chat, ChatMessage, ModerationResult, UserMetrics, Category | Поддерживает транзакционную модель (ACID) и эффективно работает с OLTP-нагрузкой |
| **Tarantool** | UserSession, ChatMessageBuffer | Обеспечивает высокую производительность за счет in-memory хранения. Обладает большей надежностью, чем Redis, потому что не теряет данные при отключении питания. Позволяет гибко конфигурировать и модифицировать БД скриптами на lua |
...
| **Clickhouse** | RawUserMetrics | Оптимизирован для OLAP-запросов и работы с большими объемами данных. Поддерживает колоночное хранение, что делает запросы быстрее. Позволяет задавать ограничения на используемые вычислительные ресурсы для аналитиков, чтобы контролировать нагрузку на БД |
...
- **Card**
  - GiST индекс по полю `address` обеспечивает эффективный поиск объявлений вблизи пользователя
  - индекс для поля `user_id` для получения объявлений определенного пользователя

- **Review**
  - составной индекс `(target_user_id, created_at)` для получения отзывов о пользователе в порядке создания
  - индекс `(author_user_id, created_at)` для поиска отзывов, оставленных пользователем

- **ChatMessage**
  - индекс `(chat_id, created_at DESC)` для выборки последних сообщений в чате

- **ModerationResult**
  - индекс `(item_id, status)` для проверки статуса модерации объявления/отзыва
  - индекс `(status, created_at)` для получения последних отклоненных/одобренных объектов
...
Пользователи чаще всего интересуются объявлениями, расположенными в пределах заданного радиуса поиска. Чтобы эффективно распределить нагрузку и ускорить геопоиск, мы шардируем данные относящиеся к пользователям (**User**, **Review**, **UserMetrics**, **RawUserMetrics**) и данные относящиеся к объявлениям (**Card**, **CardSearch**, **ModerationQueue**, **ModerationResult**) по Geohash.

При создании объявления Geohash вычисляется на основе координат (долготы и широты) из поля `address`, которое указывает местоположение объявления. При поиске система определяет геолокацию пользователя, преобразует её в Geohash и использует полученную строку для выбора шардов, в которых будет выполняться поиск.

Для эффективного хранения и масштабирования чатов мы шардируем **Chat**, **ChatMessage** и **ChatMessageBuffer** по `card_id`. Это гарантирует, что все сообщения одного чата попадут в один шард, что упрощает транзакции и поиск сообщений.
...
- **PostgreSQL** - [pgx](https://github.com/jackc/pgx)
  - производительность `pgx` лучше стандартного `database/sql` с `pq`
  - нативно реализует бинарный протокол PostgreSQL без зависимости от libpq.
  - имеет встроенный connection pool
...
Для балансировки запросов и мультиплексирования подключений будем использовать PgBouncer.

Для каждого подключения PostgreSQL создает отдельный процесс, который это подключение обслуживает, что плохо скажется на производительности СУБД од большой нагрузкой. **PgBouncer** зараннее инициирует подключения, чтобы потом их переиспользовать и не тратить вычислительные ресурсы и время на открытие соединений. Кроме того, это позволяет контролировать нагрузку на СУБД ограничив количество соединений.

**PgBouncer** позволяет также балансировать нагрузку между несколькими инстансами PostgreSQL, распределяя запросы на разные сервера.
...
- **PostgreSQL**
  - раз в сутки делаем полный физический бэкап, то есть копируем все файлы БД, используя `pg_basebackup`
  - раз в 6 часов делаем логический бэкап, используя `pg_dump`
